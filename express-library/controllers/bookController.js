...
exports.book_create_post = [
       ...
       * In order to validate the information we first convert the request to an array 
...
/**
 * We then use a wildcard (*) in the sanitizer to individually validate 
 * each of the genre array entries. The code below shows how - this 
 * translates to "sanitize every item below key genre".
 */
body("genre.*").escape(),
...
exports.book_delete_get = asyncHandler(async (req, res) => {
    ...
    if (bookInstances.length > 0) {
      ...
      // No book instances. Render the page to confirm deletion.
  ...
  exports.book_delete_post = asyncHandler(async (req, res) => {
    const bookInstances = await BookInstance.find({ 'book': req.body.bookid });
    ...
    if (bookInstances.length > 0) {
      ...
      // No book instances. Delete book and redirect to books list.
